...
import { fetchPremiumContentById } from "../../../../services/auth/api.services"; // Assuming these functions exist or will be created
...
const LockedContentDisplayPage = () => {
  ...
  const { currentUserId, isAuthLoading } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      try {
        ...
        console.log("contentdata---", response);
        ...
        const userIsCreator = data?.createdById === currentUserId;
  ...
  const handleSuccessfulSignup = (data) => {
    if (data.token) {
      ...
      toast.error(
        "Signup completed, but no token received. Please try signing in."
      );
  ...
  const handlePayment = async () => {
    if (!lockedContentData?.unlockPrice || lockedContentData.unlockPrice <= 0) {
      toast.error(
        "This content is not available for purchase or has an invalid price."
      );
      return;
    }
    if (!currentUserId) {
        ...
        {!showSigninModal && !showSignupModal && (
          <>
            ...
            <p className="text-sm text-gray-400 mb-6">
              You need an account to view or purchase this content.
            </p>
  ...
  if (!lockedContentData && !isLoading) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-sm text-gray-400 mb-6">
          This might be due to an invalid ID, a network issue, or the content
          not existing.
        </p>
        <button
          onClick={() => {
          ...
          className="mt-4 px-4 py-2 bg-orange-500 rounded hover:bg-orange-600 transition-colors"
        >
          Retry Loading
        </button>
      </div>
  ...
  const isCreator = lockedContentData?.createdById === currentUserId;
  console.log(
    "creatorId",
    lockedContentData.createdBy,
    lockedContentData.createdById,
    currentUserId,
    isCreator
  );
                ...
                {lockedContentData.unlockPrice > 0 ? (
                  <>
                    <p className="text-gray-400 mb-5">
                      ...
                      <Icons.IndianRupee className="inline-block w-4 h-4 -mt-1" />
                      {lockedContentData.unlockPrice} to unlock the full
                      content.
                    </p>
                    <button
                      onClick={handlePayment}
                  ...
                  <p className="text-gray-400">
                    This content is currently locked and not available for
                    purchase.
                  </p>
                )}
              </div>
