...
const login = async () => {
  try {
    ...
    const phone_code = await askQuestion('Enter the code you received on Telegram: ');
...
app.post('/webhook', async(req, res) => {
  ...
  const {new_chat_member, old_chat_member, invite_link, chat } = update.chat_member;

  console.log('Received update:', JSON.stringify(update, null, 2));
  
  if (update.chat_member && update.chat_member.invite_link) {
    if (old_chat_member.status !== 'member' && new_chat_member.status === 'member') {
      try {
        console.log("Invi", new_chat_member);
        
        const userId = await ChannelUser.get(invite_link.invite_link);
        ...
        console.error("Error in verifying joining.", error);
...
app.get('/users', (req, res) => {
  ...
  providedKey === expectedKey ? res.json(channelUserData) : res.status(401).send('Unauthorized');
});
...
const checkExpiredSubscriptions = async () => {
  try {
    ...
    for (const sub of expiredSubscriptions) {
      console.log(`Removing user ${sub.boughtById} from channel ${sub.telegram.channelName}`);
